...
#endif
#endif
        shortVersion?"":",\"capabilities\":" HUE_CAPABILITIES ",\"swversion\":\"5.105.0.21169\""
        );
}
...
int ICACHE_FLASH_ATTR handleListEvent(HttpdConnData *connData) {
  ...
  if (lights == NULL) {
    ...
    "\"name\":\"Remote\"}},\"swversion\":\"01041302\",\"apiversion\":\"1.17.0\",\"swupdate\":{\"updatestate\":0,\"url\":\"\",\"text\":\"\",\"notify\": false}}}",
    MAC2STR(macAddr), 
    IP2STR(&info.ip),
    IP2STR(&info.netmask),
    IP2STR(&info.gw),
...
{
  if (flashConfig.alexa_devices>0) {
